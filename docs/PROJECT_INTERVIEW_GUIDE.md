# Agentic RAG 新生智能助手：实现细节与面试追问手册

> 文档定位：这不是宣传稿，而是按当前仓库实际代码整理的实现说明、设计取舍和面试答题材料。阅读时应重点掌握“为什么这样设计”“数据如何流动”“失败时怎么处理”“哪些已经实现、哪些仍是规划”。
...
这部分是面试防守重点。主动说明边界，比被面试官追问后才承认更可信。
...
面试中可以把它概括为：**PostgreSQL 保存事实，Redis 负责协调。**
...
面试时可把它称为“GraphRAG 风格的轻量本地图检索层”，不要直接说“完整实现微软 GraphRAG”。
...
如果这些问题都能脱稿回答，已经足以应对大多数围绕该项目的中后台、AI应用工程和Agent/RAG方向面试追问。
