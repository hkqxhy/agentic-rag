...
PAIMON 当前采用“可运行 V1 基线 + 渐进式 V2 重做”的迁移策略。代码仓库必须能明确区分受支持代码、历史原型、知识治理元数据、本地原始资料和运行产物。

## 当前结构

```text
PAIMON/
├─ paimon_next/             # 可运行的 V1 基线 RAG、API、CLI 与评测代码
├─ PAIMON.py                # 旧调用方式兼容入口
├─ tests/                   # V1 单元与回归测试
├─ eval/cases/              # 小型版本化评测集
...
2. 新能力进入 `apps/` 与 `src/paimon/` 前，必须有明确职责和可执行代码，不创建表演性空目录。
3. 旧脚本仅归档，不再从生产入口调用。
...
- `python -m compileall -q PAIMON.py paimon_next tests` 通过；
