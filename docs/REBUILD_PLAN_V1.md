...
根目录旧脚本曾在迁移期放入 `legacy/`；当前主链路通过效果和性能门禁后，已从主分支删除，历史仍可通过 Git 追溯。
...
1. **部署地点**：初版开发、集成、评测和演示使用阿里云 ECS 个人免费试用 4C8G；若可选地域优先华北 2（北京），与百炼工作空间同地域。控制台预计约 600 小时，通过节省停机模式按需使用；该按量试用实例不满足中国大陆 ICP 备案条件，因此定位为阶段性工程验收环境而非长期公网生产站点。详细步骤见 [`CLOUD_DEPLOYMENT_GUIDE.md`](CLOUD_DEPLOYMENT_GUIDE.md)。
...
项目对外只陈述实际完成和实测的数据；在没有真实用户运营记录时，统一表述为“按约 100 用户规模设计并压测验证”。
