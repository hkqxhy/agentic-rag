...
4. 当前免费试用是按量付费实例，不满足中国大陆 ICP 备案条件。因此它适合部署测试、压测和短期演示，不适合宣称为长期面向公众运营的正式站点。
...
首版使用 Docker Compose，不为增加技术栈复杂度而提前引入 Kubernetes：
...
10. 短期演示可直接使用临时 HTTPS 域名或本地端口转发；若需要长期公开域名，试用结束后迁移到满足备案条件的包年包月实例。
...
如果未达标，报告中应记录实测结果和优化过程，不把目标值写成已实现值。
...
1. 阶段性展示已经完成：关闭实例，保留代码、镜像、备份、视频和报告，固定成本降为零；
